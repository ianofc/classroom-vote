...
const Index = () => {
  ...
  const submitLogin = async (e: React.FormEvent) => {
    ...
    if (error || !data.user) { setLoginError("E-mail ou senha incorretos."); } 
    else { setLoginData({ user: "", pass: "" }); await fetchEscola(data.user.id); setPhase("welcome"); }
  };

  const handleLogout = async () => {
    await supabase.auth.signOut();
    setEscolaNome("Escola"); setActiveElections([]); setVoterData(null); setPhase("auth");
  };
  ...
  const handleVote = async (votesArray: any[], currentVoterData: any) => {
    ...
    try {
      ...
      toast({ title: "Modo Offline Ativo", description: "O voto foi salvo localmente e será sincronizado quando a internet voltar.", className: "bg-orange-50 text-orange-900 border-orange-200" });
    } finally {
    ...
    const syncOfflineVotes = async () => {
      ...
      if (offlineVotes.length > 0 && navigator.onLine) {
        ...
        if (!error) {
          ...
          toast({ title: "Sincronização Concluída", description: `${offlineVotes.length} votos offline enviados para o servidor!` });
        }
      }
    };
  ...
  if (isCheckingAuth) return <div className="bg-aurora min-h-screen flex items-center justify-center"><Loader2 className="w-10 h-10 text-blue-600 animate-spin" /></div>;
    ...
    <div className="bg-aurora flex flex-col items-center justify-center min-h-screen relative overflow-hidden text-slate-900 dark:text-white transition-colors duration-300">
        ...
        <div className="w-full max-w-6xl min-h-screen md:min-h-[85vh] md:my-8 bg-white dark:bg-slate-900 md:rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-500 border border-slate-200 dark:border-slate-800">
          <div className="hidden md:flex md:w-5/12 bg-gradient-to-br from-blue-700 to-indigo-900 p-12 flex-col justify-between text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-10"><ShieldCheck className="w-10 h-10 text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]" /><h1 className="text-3xl font-black tracking-tight text-white">Classroom Vote</h1></div>
              <h2 className="text-4xl font-black leading-tight mb-6 text-white tracking-tighter">A democracia na sua escola levada a sério.</h2>
              <p className="text-blue-100 text-lg mb-12 max-w-sm font-medium">Plataforma de votação criptografada em Blockchain, simples de usar e com apuração em tempo real.</p>
              <div className="bg-white/10 backdrop-blur-sm border border-red-500/30 p-6 rounded-2xl mb-8 shadow-2xl">
                ...
                <p className="font-medium text-blue-50 italic leading-relaxed mb-6">"O sistema revolucionou a forma como elegemos os nossos líderes! A apuração na Eleição de Líderes de Sala foi instantânea e 100% à prova de fraudes."</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center font-black text-white shadow-lg border-2 border-red-400">IS</div>
                  <div><p className="font-black text-sm text-white tracking-wide">Ian Santos</p><p className="text-xs text-blue-300 mt-0.5 leading-tight">Professor, CEEPS Seabra-Ba e<br/>Criador do App Classroom Vote</p></div>
                </div>
              </div>
            </div>
            <div className="relative z-10 border-t border-red-500/30 pt-8 mt-auto">
              ...
              <div className="flex items-center gap-6"><div className="flex items-center gap-2 bg-blue-800/80 px-4 py-2.5 rounded-xl border border-blue-700 shadow-sm transition-transform hover:scale-105"><Building2 className="w-4 h-4 text-blue-400"/> <span className="text-sm font-black tracking-widest text-white">CEEPS</span></div></div>
            </div>
          </div>

          <div className="w-full md:w-7/12 p-8 md:p-16 flex flex-col justify-center bg-slate-50 dark:bg-slate-900 relative">
            <div className="max-w-md w-full mx-auto">
              <div className="text-center md:text-left mb-10"><h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Acesso Restrito</h2><p className="text-slate-500 dark:text-slate-400 font-medium mt-2">Bem-vindo de volta! Inicie sessão para gerir o painel da sua escola.</p></div>
              <form onSubmit={submitLogin} className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><User className="w-5 h-5 absolute left-4 top-3.5 text-slate-400" /><input required autoFocus type="email" placeholder="diretoria@escola.com.br" className="w-full pl-12 p-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent text-slate-800 dark:text-white transition-all shadow-sm" value={loginData.user} onChange={e => setLoginData({...loginData, user: e.target.value})} /></div>
                </div>
                <div className="space-y-1.5">
                ...
                <button type="submit" disabled={isLoggingIn} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black uppercase tracking-widest py-4 rounded-xl flex justify-center items-center gap-2 mt-4 transition-all shadow-lg shadow-blue-600/30 disabled:opacity-50">{isLoggingIn ? <Loader2 className="w-5 h-5 animate-spin" /> : "Entrar no Sistema"}</button>
              </form>
            </div>
          </div>
        </div>
      ...
      {phase === "welcome" && (
        <div className="glass-panel p-12 rounded-3xl flex flex-col items-center max-w-lg w-[90%] text-center animate-in fade-in zoom-in duration-1000 z-10">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-slate-800 dark:text-white uppercase tracking-tighter leading-tight mb-2">Bem-vindo às<br/>Eleições</h1>
          ...
          <p className="mt-6 text-sm text-slate-500 dark:text-slate-400 font-medium flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Sincronizando Cadeia Criptográfica...</p>
        </div>
      )}

      {/* ================= FASE 3: MESA DO MESÁRIO ================= */}
      {phase === "identify" && (
        <div className="w-full h-full animate-in fade-in duration-700 flex flex-col items-center pt-24 z-10 px-4">
          <div className="fixed top-4 left-4 right-4 md:left-auto md:right-auto md:w-[700px] bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-3 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex justify-between items-center z-40">
            <div className="flex items-center gap-3">
              <div className="bg-blue-100 dark:bg-blue-900/50 p-2.5 rounded-full"><ShieldCheck className="w-5 h-5 text-blue-600 dark:text-blue-400" /></div>
              <div className="text-left"><p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none mb-0.5">Mesa Receptora</p><p className="text-sm font-black text-slate-800 dark:text-white leading-tight truncate max-w-[150px] md:max-w-[250px]">{escolaNome}</p></div>
            </div>
            <div className="flex items-center gap-2">
               <button onClick={() => setPhase("admin")} className="text-xs font-bold bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 py-2 px-3 rounded-lg transition-colors border border-blue-100 dark:border-blue-800">Painel de Gestão</button>
          ...
          <div className="w-full max-w-[700px] mt-10">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium mt-2">Existem <strong className="text-blue-600 dark:text-blue-400">{activeElections.length}</strong> eleições abertas. O sistema decidirá a cédula automaticamente.</p>
            </div>

            <div className="relative mb-6">
              <Search className="w-6 h-6 absolute left-5 top-4 text-slate-400" />
              <input type="text" placeholder="Digite o nome do aluno para liberar a urna..." className="w-full pl-14 pr-6 py-4 text-lg bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:border-blue-500 dark:focus:border-blue-500 text-slate-800 dark:text-white shadow-lg transition-all" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} autoFocus />
                ...
                {searchResults.length === 0 && !searching ? (
                  ...
                  searchResults.map(student => (
                    <button key={student.id} onClick={() => selectVoter(student)} className="w-full p-4 border-b border-slate-100 dark:border-slate-700/50 flex items-center justify-between hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors text-left">
                      <div><p className="font-bold text-slate-800 dark:text-white text-lg">{student.name}</p><p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Turma: {student.turmas?.name}</p></div>
                      {student.candidate_role && (<div className="bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5"><UserCheck className="w-3 h-3" /> PERFIL: {student.candidate_role}</div>)}
                    </button>
                  ))
                )}
              </div>
      ...
      {phase === "setup" && voterData && urnaPayload && (
        <div className="w-[90%] max-w-md glass-panel rounded-3xl p-8 text-center animate-in fade-in slide-in-from-bottom-4 duration-500 z-10">
          <div className="space-y-6">
            ...
            <div><h1 className="text-2xl font-black text-slate-800 dark:text-white leading-tight">{voterData.name}</h1><p className="text-sm font-bold text-slate-500 mt-1 uppercase tracking-widest">Turma: {urnaPayload.name}</p></div>
            ...
            <button onClick={handleStartVoting} className="w-full py-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black uppercase tracking-widest transition-all shadow-lg hover:shadow-blue-500/30">Liberar Urna</button>
      ...
      {phase === "voting" && urnaPayload && (
        <div className="w-full animate-in fade-in duration-500 z-10">
          ...
          <button onClick={() => setPhase("admin")} className="fixed bottom-6 right-6 w-12 h-12 rounded-full glass-panel flex items-center justify-center transition-all hover:scale-110 z-50 text-slate-500 dark:text-slate-400 shadow-lg"><ShieldCheck className="w-5 h-5" /></button>
        </div>
      )}
