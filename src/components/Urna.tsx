...
const Urna = ({ turma, onVoteConfirmed, onBack }: UrnaProps) => {
  ...
  const handleNumber = (num: number) => {
    if (isLocked) return; // BLOQUEIO ANTI-SPAM
    if (digits.length < 2) {
      playUrnaSound('key');
  ...
  const handleConfirma = () => {
    ...
    processVote({ role: currentElection.nome, number: parseInt(digits), type: voteType });
  };
  ...
  const processVote = (voteData: any) => {
    ...
    if (currentRoleIndex < electionsSequence.length - 1) {
      ...
      setIsLocked(true); // TRANCA A URNA COMPLETAMENTE AQUI
      playUrnaSound('end');
      ...
      setTimeout(() => {
        ...
        // O index.tsx vai desmontar a urna agora, não precisa resetar estado
      }, 4000); 
    }
  };
          ...
          {showEndAnim ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white z-10">
              ...
              <div className="absolute bottom-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Protocolo Criptográfico de Segurança</p>
            ...
            <div className="h-full flex flex-col">
              <div className="mb-6">
                ...
                {currentElection.tipo === 'turma' && <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Candidatos limitados à sua turma</p>}
              </div>
              <div className="flex-1 flex gap-6">
                <div className="flex-1 space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-2">
                      <div className="w-12 h-14 border-2 border-slate-800 flex items-center justify-center text-3xl font-black shadow-inner bg-slate-50">{digits[0] || ""}</div>
                      <div className="w-12 h-14 border-2 border-slate-800 flex items-center justify-center text-3xl font-black shadow-inner bg-slate-50">{digits[1] || ""}</div>
                    </div>
                  </div>
                  <div className="space-y-4 pt-4">
                    <div className="flex items-start gap-4 min-h-[3rem]">
                      ...
                      <p className="text-xl font-black text-slate-800 uppercase leading-none">{digits.length === 2 ? (candidate ? candidate.name : "VOTO NULO") : ""}</p>
                    </div>
                    {candidate?.vice_name && (
                      <div className="flex items-start gap-4"><span className="text-sm font-bold text-slate-600 uppercase w-20 pt-1">Vice:</span><p className="text-lg font-bold text-slate-700 uppercase leading-none">{candidate.vice_name}</p></div>
                    )}
                  </div>
                </div>
                <div className="w-32 h-40 border-2 border-slate-800 flex flex-col items-center justify-end bg-slate-100 p-2 shadow-inner">
                  ...
                  <p className="text-[10px] font-bold text-slate-500 uppercase text-center w-full truncate border-t border-slate-300 pt-1">{digits.length === 2 && candidate ? "CANDIDATO" : "FOTO"}</p>
                </div>
              </div>
              <div className="mt-auto border-t-2 border-slate-800 pt-3">
                <p className="text-xs font-bold text-slate-800">Aperte a tecla:</p>
                <div className="flex gap-4 mt-1">
                  <p className="text-xs font-bold text-slate-600"><strong className="text-green-600">VERDE</strong> para CONFIRMAR</p>
                  <p className="text-xs font-bold text-slate-600"><strong className="text-orange-500">LARANJA</strong> para REINICIAR</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="w-full md:w-80 bg-slate-800 p-6 md:p-8 rounded-xl shadow-2xl border-b-8 border-slate-900 flex flex-col gap-6">
          <div className="grid grid-cols-3 gap-3 md:gap-4">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
              <button key={num} onClick={() => handleNumber(num)} disabled={isLocked} className="bg-slate-900 text-white text-xl md:text-2xl font-black py-4 rounded-md shadow-[0_4px_0_rgb(15,23,42)] active:translate-y-1 hover:bg-slate-700 transition-all border border-slate-700 disabled:opacity-50">{num}</button>
            ))}
            <div className="col-start-2">
              <button onClick={() => handleNumber(0)} disabled={isLocked} className="w-full bg-slate-900 text-white text-xl md:text-2xl font-black py-4 rounded-md shadow-[0_4px_0_rgb(15,23,42)] active:translate-y-1 hover:bg-slate-700 transition-all border border-slate-700 disabled:opacity-50">0</button>
            </div>
          </div>
          <div className="flex gap-2 md:gap-3 mt-4 pt-4 border-t border-slate-700">
            <button onClick={handleBranco} disabled={isLocked} className="flex-1 bg-white text-slate-800 text-[10px] md:text-xs font-black uppercase py-4 rounded shadow-[0_4px_0_rgb(203,213,225)] active:translate-y-1 transition-all hover:bg-slate-50 border border-slate-300 disabled:opacity-50">Branco</button>
            <button onClick={handleCorrige} disabled={isLocked} className="flex-1 bg-orange-500 text-slate-900 text-[10px] md:text-xs font-black uppercase py-4 rounded shadow-[0_4px_0_rgb(194,65,12)] active:translate-y-1 transition-all hover:bg-orange-400 border border-orange-600 disabled:opacity-50">Corrige</button>
            <button onClick={handleConfirma} disabled={isLocked} className="flex-1 bg-green-500 text-slate-900 text-[10px] md:text-xs font-black uppercase py-4 rounded shadow-[0_4px_0_rgb(21,128,61)] active:translate-y-1 transition-all hover:bg-green-400 border border-green-600 disabled:opacity-50">Confirma</button>
          </div>
        </div>
      </div>
    </div>
