...
export default function TurmaSelection({ onSelect, onAdmin }: { onSelect: (t: any) => void, onAdmin: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 w-full max-w-md mx-auto">
      <div className="glass-panel w-full p-8 rounded-3xl space-y-8">
        ...
        {loading ? (
          <div className="flex flex-col items-center py-10 text-blue-600 dark:text-blue-400">
        ...
        <button 
          onClick={onAdmin} 
          className="w-full flex items-center justify-center gap-2 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors mt-4 border-t border-slate-200 dark:border-slate-700/50"
        >
