...
import { 
  ...
  Lock, Trash2, GraduationCap, Printer, BarChart3, CheckCircle2, AlertTriangle, 
  User, CheckSquare, Maximize, ActivitySquare, ChevronLeft, ChevronRight, 
  Download, Loader2, Trophy, Flame, TrendingUp, Users, Target, 
  Image as ImageIcon, Contact, Database, UserCheck 
} from "lucide-react";
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-6 bg-slate-50 text-slate-900">
      ...
      <div className="w-full max-w-[1200px] flex flex-wrap gap-2 bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm mb-6 justify-center">
        {(["apuracao", "midias", "reports", "eleicoes", "turmas", "candidatos", "admins", "perfil"] as Tab[]).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-xs font-bold transition-all ${activeTab === tab ? "bg-blue-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-50"}`}>
            ...
            {tab === "candidatos" && <UserCheck className="w-4 h-4" />}
      ...
      <div className="w-full max-w-[1200px]">
        ...
        {/* COMPONENTES EXTERNOS MANTIDOS A 100% */}
        {activeTab === "candidatos" && <div className="animate-in fade-in duration-300"><ManageCandidatos /></div>}
