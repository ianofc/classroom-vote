...
import { 
  ...
  Image as ImageIcon, Contact, Database 
} from "lucide-react";
...
const AdminPanel = ({ turma, onBack, onTurmasChanged }: AdminPanelProps) => {
  // Estados Escolares e de Configuração
    ...
    try {
      ...
      if (userData?.user) {
        ...
        if (adminData?.escolas) {
          escolaData = Array.isArray(adminData.escolas) ? adminData.escolas[0] : adminData.escolas;
        }
      ...
      const [votesData, turmasData, candidatesRes, logsData, eleicoesData, studentsData] = await Promise.all([
        ...
        fetchEverything('students')
      ]);

      const sortedVotes = (votesData as ExtendedVoteRecord[]).sort((a, b) => 
      ...
      if (eleicoesData) {
        ...
        // Inclui "Eleições Legadas" se houver votos sem ID de eleição vinculado
  ...
  useEffect(() => {
    if (["reports", "apuracao", "logs", "midias"].includes(activeTab)) {
      fetchAllData();
    }
  }, [activeTab]);
  ...
  // --- MEMOS (Cálculos de Relatórios e Dashboard) ---
  const estatisticasEscola = useMemo(() => {
    ...
    return {
      ...
      abstencao: (totalEleitores > 0 ? 100 - comparecimento : 0).toFixed(1),
  ...
  const downloadSnapshot = async () => {
    try {
      ...
      const [vData, tData, sData, eData, lData] = await Promise.all([
        fetchEverything('votes'), fetchEverything('turmas'), fetchEverything('students'),
        fetchEverything('eleicoes'), fetchEverything('admin_logs')
      ]);
      
      const snapshot = {
        app: "Classroom Vote Enterprise",
        version: "1.0",
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `snapshot_seguranca_${escolaNome.replace(/\s+/g, '_')}_${new Date().getTime()}.json`;
      link.click();
      ...
      if (userData?.user?.email) {
        await supabase.from('admin_logs').insert({ admin_email: userData.user.email, acao: "BACKUP", detalhes: "Foi gerado um Snapshot JSON da Base de Dados inteira." });
      }
    } catch(e) {
  ...
  const handleDeleteVote = async (id: string, voterName: string) => {
    ...
    if (!error) {
      ...
      const { data: userData } = await supabase.auth.getUser();
      if (userData?.user?.email) {
        await supabase.from('admin_logs').insert({ admin_email: userData.user.email, acao: "EXCLUSÃO DE VOTO", detalhes: `Voto de ${voterName} deletado da auditoria.` });
      }
      toast({ title: "Sucesso", description: "Voto excluído com sucesso." });
    } else {
      toast({ title: "Erro", description: "Não foi possível excluir o voto.", variant: "destructive" });
    }
  };
  ...
  const printDashboardReport = () => {
    ...
    if (apuracaoResults && apuracaoResults.length > 0) {
      ...
      rolesHtml = `<p style="text-align: center; color: #666; margin-top: 40px;">Nenhum dado de votação encontrado.</p>`;
    }
      ...
      <div className="w-full max-w-[1200px] flex flex-wrap gap-2 bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm mb-6 justify-center">
        {(["apuracao", "midias", "reports", "eleicoes", "turmas", "admins", "logs", "perfil"] as Tab[]).map((tab) => (
          <button 
            ...
            className={`flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-xs font-bold transition-all ${activeTab === tab ? "bg-blue-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-50"}`}
          >
        ...
        {activeTab === "apuracao" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 mt-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium">Selecione os filtros abaixo para ver a contagem de votos.</p>
                </div>
              </div>
              <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                <select 
                  className="w-full md:w-64 p-3 border-2 border-slate-200 rounded-xl text-sm font-bold bg-slate-50 outline-none focus:border-blue-500" 
                  value={apuracaoEleicaoId} 
                  onChange={e => setApuracaoEleicaoId(e.target.value)}
                >
                ...
                <button 
                  ...
                  disabled={isPrinting || (!isEleicaoGlobal && !apuracaoTurmaId)} 
                  className="w-full md:w-auto bg-slate-900 hover:bg-slate-800 text-white px-5 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
                >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-bold text-slate-500 uppercase">Brancos</p>
                <p className="text-3xl font-black text-slate-600 mt-1">{apuracaoOverview.brancos}</p>
              </div>
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                <p className="text-xs font-bold text-slate-500 uppercase">Nulos</p>
                <p className="text-3xl font-black text-orange-500 mt-1">{apuracaoOverview.nulos}</p>
              </div>
            </div>

            {reportLoading ? (
              <div className="py-12 text-center text-slate-400 font-bold animate-pulse">Calculando...</div>
                  ...
                  <div key={idx} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
                    ...
                    <div className="p-6 flex-1 space-y-6">
                      {result.candidateResults.length === 0 ? (
                        <p className="text-sm text-slate-400 text-center py-4">Nenhum candidato.</p>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="relative w-full md:w-64">
                ...
                <input 
                  ...
                  className="w-full pl-9 p-2.5 border rounded-lg text-sm bg-slate-50 outline-none focus:border-blue-500" 
                  value={midiaSearch} 
                  onChange={e => setMidiaSearch(e.target.value)} 
                />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
                ...
                <button onClick={() => setShowVotes(!showVotes)} className="text-[10px] font-bold bg-white border px-3 py-1.5 rounded-md hover:bg-slate-100 flex items-center gap-1 shadow-sm transition-colors">
                  ...
                  {showVotes ? "OCULTAR VOTOS" : "REVELAR VOTOS"}
                </button>
              </div>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 sticky top-0 shadow-sm z-10">
                      <tr className="text-[10px] text-slate-500 uppercase">
                        ...
                        <th className="p-4 font-black text-center">Voto</th>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm animate-in fade-in duration-300">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6 border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div><h2 className="text-xl font-black text-slate-800">Logs de Segurança do Sistema</h2><p className="text-sm text-slate-500 font-medium">Auditoria rigorosa. Registro imutável de quem fez o quê.</p></div>
              </div>
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {systemLogs.length === 0 && <p className="text-center text-sm text-slate-400 py-10">Nenhuma ação sensível registrada recentemente.</p>}
              {systemLogs.map(log => (
